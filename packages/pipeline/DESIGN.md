...
How `pipeline.matter_signals` turns a matter brief into a ranked shortlist of firms, and where the current implementation is honest about not knowing.
...
Evidence-first ordering is deliberate. Global rank alone puts one-case firms at the top (see the ranking caveat in the root README), which is useless for someone actually choosing counsel.
...
**This signal is currently wrong, and the code path is live.** It reads `PredDefWinProba` as P(defendant wins). On the sample it isn't:
...
Replacing it means computing an empirical win rate from `Winner`, shrunk toward the case-type base rate, and returning `unknown` below a sample-size floor. Until that lands, treat the win rate and lift in the Matters UI as placeholder.
...
- Predicted win rate — see the caveat above
...
Named here so nobody reads the list above as a description of finished work.

- **Shrinkage.** No prior anywhere. A firm at n=1 can report 0% or 100%. This is the single largest correctness gap.
...
- **Rerank.** The signals are computed and displayed but not folded back into the ordering; ranking is still evidence count then global rank.
